...
class cNMF():
    ...
    def get_norm_counts(self, counts, tpm,
        ...
        if zerocells.sum()>0:
            ...
            raise Exception('Error: %d cells have zero counts of overdispersed genes. E.g. %s. Filter those cells and re-run or adjust the number of overdispersed genes.' % (zerocells.sum(), ', '.join(examples[:4])))
